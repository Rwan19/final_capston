...
import com.example.myapplication.util.removeItem
...
class NotificationsViewModel(private val myUserID: String) : DefaultViewModel() {
    ...
    private fun loadNotifications() {
        dbRepository.loadNotifications(myUserID) { result: ResponseStateResult<MutableList<UserNotification>> ->
            onResult(userNotificationsList, result)
            if (result is ResponseStateResult.Success) result.data?.forEach { loadUserInfo(it) }
        }
    }

    private fun loadUserInfo(userNotification: UserNotification) {
        dbRepository.loadUserInfo(userNotification.userID) { result: ResponseStateResult<UserInfo> ->
